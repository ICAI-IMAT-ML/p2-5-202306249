...
class LogisticRegressor:
    ...
    def fit(
        ...
        for i in range(num_iterations):  # Fill the None here
            ...
            dw = np.dot(X.T, (y_hat - y)) / m  # Derivative w.r.t. the coefficients
            db = np.sum(y_hat - y) / m  # Derivative w.r.t. the intercept
    ...
    def elasticnet_regularization(self, dw, m, C, l1_ratio):
        ...
        dw_lasso = (C / m) * np.sign(self.weights)  # L1 regularization
        dw_ridge = (C / m) * self.weights           # L2 regularization    
        elasticnet_gradient = l1_ratio * dw_lasso + (1 - l1_ratio) * dw_ridge
